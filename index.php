...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'type') {
        ...
        $db->prepare('INSERT INTO element_types(system_id,name,spacing) VALUES(?,?,?)')->execute([$systemId, trim($_POST['name']), $spacing]);
    }
    if ($action === 'element') {
...
<?php elseif ($currentPage === 'types'): ?><section class="panel"><h2>Tipos de elemento</h2><form method="post"><input type="hidden" name="action" value="type"><input type="hidden" name="return_page" value="types"><input type="hidden" name="system_id" value="<?= $systemId ?>"><input name="name" required placeholder="Ex.: Verbo"><select name="spacing" aria-label="Espaçamento do tipo"><option value="with_space" selected>COM espaço</option><option value="without_space">SEM espaço</option></select><button>Adicionar</button></form><ul><?php foreach ($items as $type): ?><li><small>ID <?= $type['id'] ?> · <?= ($type['spacing'] ?? 'with_space') === 'without_space' ? 'SEM espaço' : 'COM espaço' ?></small><i></i><?= htmlspecialchars($type['name']) ?></li><?php endforeach; ?></ul><?= pagination($listPage, $listLastPage, 'types', $systemId) ?></section>
